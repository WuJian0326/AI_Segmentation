...
class FCT(nn.Module):
    ...
    def forward(self,x):
        ...
        out7_adjusted = F.interpolate(out7, size=out9.shape[2:], mode='bilinear', align_corners=False)
        out8_adjusted = F.interpolate(out8, size=out9.shape[2:], mode='bilinear', align_corners=False)


        return out7_adjusted, out8_adjusted, out9
def init_weights(m):
    """
